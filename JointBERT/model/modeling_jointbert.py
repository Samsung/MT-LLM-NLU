        ...
        if contrastive == True:
            ...
            for idx in sentence_id:
                ...
                if len(indices) == 2:
                    ...
                    assert language_id[idx_1] != language_id[idx_2]
                    repr_1 = self.projection_layer(pooled_output[idx_1])
                    repr_2 = self.projection_layer(pooled_output[idx_2])
                    pair_loss = F.mse_loss(repr_1, repr_2, reduction='mean')
                    sieving_contrastive_loss.append(pair_loss.detach().numpy().item())
            ...
            #print("Contrastive loss: ", contrastive_loss.detach().numpy().item())
        ...
        if intent_label_ids is not None:
            if self.num_intent_labels == 1:
                ...
                if sieving == True:
                    ...
                    sieving_intent_loss = sieving_intent_loss_fct(intent_logits.view(-1, self.num_intent_labels), intent_label_ids.view(-1)).detach()
        ...
        if not self.args.no_domains:
            if domain_label_ids is not None:
                if sieving == True:
                    ...
                    sieving_domain_loss = sieving_domain_loss_fct(domain_logits.view(-1, self.num_domain_labels), domain_label_ids.view(-1)).detach()
